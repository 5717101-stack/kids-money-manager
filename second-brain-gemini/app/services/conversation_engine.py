...
class ConversationEngine:
    ...
    def initialize(self, user_profile: Dict[str, Any] = None, drive_memory_service=None):
        ...
        # Use 2.5 Flash for conversation — best balance of speed + accuracy:
        # - Pro (2.5-pro) is too slow: 5-15s per API call due to "thinking"
        # - Flash 2.0 is too unreliable: hallucinates names, skips tool calls
...
כללי התנהגות:
══════════════════════════════════════════════════════
1. 🔴 לשאלות על אנשים — חובה להשתמש בכלי search_person! אל תנסה לענות מהזיכרון או מה-system instruction. תמיד קרא ל-search_person כדי לברר מי באמת קיים בבסיס הידע ומה הנתונים שלו. גם אם אתה "בטוח" שאתה יודע — השתמש בכלי.
